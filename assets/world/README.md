...
## Quiet courtyard vignettes and ambient dialogue

The cast rotates through six 28-second scenes: boy alone; guardian and mage by the fire; girl alone; berserker and gladiator by the fire; pawn alone; dark knight on watch. Visitors enter, idle, and leave before the next scene; only one speech bubble appears at a time. Existing gate sentries stay at their posts. Motion pause freezes movement, scene changes, and dialogue together. Reduced motion shows a single resting resident without automatic speech.

`tools/build-courtyard-cast.py --game-root C:/OneStep/tiny_defense` exports both run and idle atlases with a shared crop per character so their size stays consistent. `cast/dialogue.json` contains website-only ambient lines in KO/EN/JA, not quotations from the game. Characterization draws from `Loc.Season.cs` (`nemesis.hero.*`) and `Loc.Story.cs` (`story.reply.*` and the boy's dialogue): protective guardian, eager berserker, playful gladiator, confident mage, restrained dark knight, caring boy. No distinct source voice was established for the girl, so her line remains neutral rather than inventing a backstory.
